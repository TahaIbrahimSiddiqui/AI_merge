...
def hierarchical_llm_match(
    ...
    for level_idx, level_cols in enumerate(hierarchy):
        ...
        left_match = (
            ...
            .query(f"{left_id_col} in @unmatched_left")
        )
    ...
    if use_llm and unmatched_left and unmatched_right:
        ...
        for _, row in right_unmatched.iterrows():
            candidate_str = " | ".join([
                f"{col}: {row[col]}" for col in display_cols if col in row and pd.notna(row[col])
            ])
        ...
        # Process LLM results
        confidence_threshold = 70  # Adjustable threshold
        llm_matches = 0
        for i, (best_idx, best_candidate, confidence) in enumerate(results):
            if best_idx >= 0 and confidence >= confidence_threshold:
                lid = left_id_list[i]
